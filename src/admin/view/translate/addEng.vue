	...
	<div class="addGloss">
		<div class="card">
			...
			<div class="card no-border">
				<div class="card-body">
					<h5 class="card-title">Tambah Terjemahan Inggris - Indonesia</h5>
					<form v-for="kata in engIn" :key="kata.id">
...
export default {
	...
	created() {
		...
		this.postUrl = mainUrl + "/postEng";
	},

	computed: {
		...mapMultiRowFields(["engIn"]),
	},
	components: {
		BSpinner,
	},
	...
	methods: {
		...mapMutations(["addEngin"]),

		async onSubmit() {
			const checker = store.state.engIn[0].judul_artikel;
			if (checker === "" || store.state.engIn[0].isi_artikel === "") {
				alert("Isi semua data!!");
			} else {
				try {
					this.loading = true;
					const check = this.$store.state.engIn;
